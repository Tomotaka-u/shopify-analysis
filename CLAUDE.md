...
## モデル使い分けポリシー
- **Sonnet**: ルーティン処理（スクリプト実行、データ埋め込み、デプロイ、ファイル操作）
- **Opus Agent**: 分析・インサイト執筆（ビジネス洞察、So What?、提言、異常値の発見）
- 週次・月次レポート作成は `/weekly-report` スキルに従い、Sonnetで全体を回してインサイト部分のみOpus Agentに委譲する
